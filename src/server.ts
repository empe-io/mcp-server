...
server.addTool({
    ...
    description: "Starts the verification process and generates a QR code for users to scan. Returns a state ID and QR code URL. ALWAYS call this tool first before checking results. After calling this tool, immediately call check_verification_status tool with the returned state ID. This tool has one argument (args) 'endpoint' that is required that specifies the verification endpoint to use (e.g., 'fairdrop').",
    parameters: z.object({
...
server.addTool({
    ...
    description: "Starts the verification process and generates a QR code for users to scan. Returns a state ID and QR code URL. ALWAYS call this tool first before checking results. After calling this tool, immediately call check_verification_status tool with the returned state ID. This tool has one argument (args) 'vpQueryId' that is required that specifies the verification vpQuery to use.",
    parameters: z.object({
...
server.addTool({
    ...
    description: "Create a credential offering with flexible recipient options. This is the foundational method that can create both targeted (specific recipient) and open (anyone can claim) offerings. Most use cases should use the specialized createTargetedOffering or createOpenOffering methods instead. This function will return the qr_code_url that should be shown somehow to the user. This function has 3 parameters: type, credentialSubject, and recipientDid. The type is the schema type to offer, credentialSubject is the actual data to include in the credential, and recipientDid is the DID of the recipient (if any).",
    parameters: z.object({
